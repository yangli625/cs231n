...
def rnn_backward(dh, cache):
    ...
    for i in reversed(range(T)):
        ...
        if (i == 0):
            ...
            h_prev = h[:,i-1,:]
        dx[:,i,:], dh_prev, dWxi, dWhi, dbi = rnn_step_backward(dh_curr, (x[:,i,:], h_prev, Wx, Wh, b, h[:,i,:]))
        dWx += dWxi
        dWh += dWhi
        db += dbi
...
def word_embedding_backward(dout, cache):
    ...
    x, W = cache
    dW = np.zeros(W.shape)
    np.add.at(dW, x.flatten(), dout.reshape((-1, dW.shape[1])))
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return dW
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    di = dnext_c * g
    dg = dnext_c * i
    da = np.column_stack((di * i * (1 - i), df * f * (1 - f), do * o * (1 - o), dg * (1 - g**2)))
    db = np.sum(da, axis=0)
    dx = da.dot(Wx.T)
    dWx = x.T.dot(da)
    dprev_h = da.dot(Wh.T)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    prev_c = np.zeros((N,H))
    for i in range(T):
        h[:,i,:], c[:,i,:], _ = lstm_step_forward(x[:,i,:], prev_h, prev_c, Wx, Wh, b)
        prev_h = h[:,i,:]
        prev_c = c[:,i,:]
...
def lstm_backward(dh, cache):
    ...
    for i in reversed(range(T)):
        if (i > 0):
            cache_i = (x[:,i,:], h[:,i-1,:], c[:,i-1,:], Wx, Wh, b)
        else:
            cache_i = (x[:,i,:], h0, np.zeros((N,H)), Wx, Wh, b)
            
        dnext_c = dprev_c
        dnext_h = dh[:,i,:] + dprev_h
        dx[:,i,:], dprev_h, dprev_c, dWxi, dWhi, dbi = lstm_step_backward(dnext_h, dnext_c, cache_i)
