...
def batchnorm_backward(dout, cache):
    ...
    dgamma = np.sum((x - mean) * var_inv * dout, axis=0)
    
    tmp = dout * gamma * var_inv - np.sum(dout * gamma * (x - mean), axis=0) * (var_inv**2) * var_inv / N * (x - mean)
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = (N * dout - np.sum(dout, axis=0) - (x - mean) * (var_inv ** 2) * np.sum(dout*(x - mean), axis=0)) * var_inv * gamma / N
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        mask = (np.random.uniform(0.0, 1.0, x.shape) < p) / p
        out = np.copy(x)
        out *= mask
        #######################################################################
        #                           END OF YOUR CODE                          #
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for n in range(N):
        for f in range(F):
            for i in range(H1):
                for j in range(W1):
                    out[n, f, i, j] = np.sum(x[n, :, (i*stride):(i*stride+HH), (j*stride):(j*stride+WW)] * w[f, :, :, :]) + b[f]
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for i in range(H1):
                for j in range(W1):
                    ind = np.argmax(x[n, c, (i*stride):(i*stride + pool_height), (j*stride):(j*stride+pool_width)])
                    tmp = dx[n, c, (i*stride):(i*stride + pool_height), (j*stride):(j*stride+pool_width)]
                    tmp[np.unravel_index(ind, tmp.shape)] = dout[n, c, i, j]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
